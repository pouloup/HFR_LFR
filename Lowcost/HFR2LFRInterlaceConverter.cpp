...
HFR2LFRInterlaceConverter::~HFR2LFRInterlaceConverter()
{
}


void HFR2LFRInterlaceConverter::processFrame()
{
	...
	medianBlur(opticalFlowNormMap, opticalFlowNormMapMedian, 5);
	medianBlur(opticalFlowAngleMap, opticalFlowAngleMapMedian, 5);
	...
	for(int pixelRow = 0 ; pixelRow < m_height ; pixelRow++)
	{
		for(int pixelCol = 0 ; pixelCol < m_width ; pixelCol++)
		{
			...
			else
			{
				...
				// Calculer le filtre gaussien à y appliquer (params dépendent du flot optique)
				int shiftRow = pixelRow - filter.rows/2;	// passage entre repère image et repère filtre
...
void HFR2LFRInterlaceConverter::computeNormAndAngleOpticalFlow(const Mat & opticalFlowMap, Mat & opticalFlowNormMap, Mat & opticalFlowAngleMap)
{
...
void HFR2LFRInterlaceConverter::getMotionBlurFilter(float norm, float angle, Mat & filter)
{
