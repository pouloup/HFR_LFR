/**
 * Authors: Anthony ARCH - Pierre-Henri DAUVERGNE - J閞閙y VIGOUR (ESIR)
**/
...
int HFR2LFRConverter::convert(unsigned int nbFrames /*= 0*/, unsigned int startFrame /* = 0*/)
{
	...
	gpum_prevFrameBGR.upload(m_prevFrameBGR);
	gpum_currentFrameBGR.upload(m_currentFrameBGR);
	gpum_nextFrameBGR.upload(m_nextFrameBGR);

	// Convertir images en niveaux de gris
	cvtColor(gpum_prevFrameBGR,    gpum_prevFrame,    CV_BGR2GRAY);
	cvtColor(gpum_currentFrameBGR, gpum_currentFrame, CV_BGR2GRAY);
	cvtColor(gpum_nextFrameBGR,    gpum_nextFrame,    CV_BGR2GRAY);

	gpum_prevFrame.download(m_prevFrame);
	gpum_currentFrame.download(m_currentFrame);
	gpum_nextFrame.download(m_nextFrame);
	...
	while(nbFrames == 0 || cpt <= nbFrames)
	{
		...
		// Cr閑r la frame finale � partir des 3 frames HFR
		processFrame();
		...
		// La frame suivante (n+1) devient la frame pr閏閐ente
		m_prevFrameBGR = m_nextFrameBGR.clone();
		m_prevFrame = m_nextFrame.clone();

		// R閏up閞er la frame principale (n+2) et la frame suivante  (n+3)
